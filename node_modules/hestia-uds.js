...
try {
	...
	if (typeof port !== 'string' || !/(\/hestia-nodeJS\/).+\-[0-9]+(.sock)$/.test(port))
		throw false

	if (/(-0.sock)$/.test(port) && instance > 0)
		process.env.PORT = port.substring(0, port.length - '0.sock'.length) + instance + '.sock'

 	if (fs.statSync(process.env.PORT)?.isSocket())
